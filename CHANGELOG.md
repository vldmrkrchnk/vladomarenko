...
- [x] **Fix duplicate replies / "Эхо" / "повторяешься" false accusations** (`src/bot.ts:818-843`): webhook handler previously awaited the full Grok stream (5-15s) before returning 200 to Telegram. Telegram treats a slow handler as failed and retries the webhook → duplicate `bot.handleUpdate` calls → duplicate replies → bot sees the same user text twice in its history and accuses the user of repeating themselves. Now: 200 is returned immediately, `bot.handleUpdate` runs asynchronously with error logging.
